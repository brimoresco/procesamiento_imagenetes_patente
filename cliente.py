...
if x == '1':
    ...
    if b == '2':
        libro = input('Ingrese el autor del libro: ')
    ...
    if b == '3':
        libro = input('Ingrese el país del libro: ')
    ...
    # Buscar por anio FUNCIONA
    if b == '4':
        libro = input('Ingrese el año que desea buscar: ')
        condicion = input(
    f"""e) Solo libros del año {libro}
    a) Libros anteriores al año {libro}
    p) Libros posteriores al año {libro}
    Ingrese una condicion: """)
    ...
    # Buscar por paginas FUNCIONA
    if b == '5':
        libro = input('Ingrese la cantidad de páginas: ')
        condicion = input(f"""Ingrese una condicion:
        e) Libros de {libro} paginas. 
        a) Libros con menos de {libro} paginas.
        p) Libros con mas de {libro} paginas. """)
        resp = requests.get(url + '/libros', params={"page":libro, "condicion":condicion})
        data_dict = resp.json()
        if len(data_dict) == 0:
            print('No hay libros')
        for i in data_dict:
    ...
    if b == '6':
        libro = input('Ingrese el lenguaje del libro: ')
...
if x == '3':
    ...
    nuevo_libro = {
        ...
        "title": str(titulo),
        "year": int(anio)}
    resp = requests.put(url + "/insertar", params={"libro":nuevo_libro})
    print(resp.status_code)
